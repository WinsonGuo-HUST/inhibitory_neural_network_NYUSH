...
Mr      = param.Mr;
duration = param.duration*1000; %3*1000
...
delay_steps = ceil(3 / dt);    % transfer time delay to steps
...
for step = 2:duration/dt
    ...
    vs(rind_s)=vs(rind_s) + dt * (ex_s(step,rind_s)*s_exs + s_se*hs(1,rind_s)/tau_se);
    ...
    if spikecount_s>0
        ...
        spikeind = nsarray(sind_s)*sl+spike_s(1,sind_s)+1;
