...
tau_es  = param.tau_es;
dt      = param.gridsize;
ref_e   = param.tau_re;     % refractory period
ref_i   = param.tau_ri;
ref_s   = param.tau_rs;
M       = param.M;      % spike threshold
Mr      = param.Mr;     % inhibitory reversal potential
duration = param.duration*1000; % transfrom s to ms
...
som_delay = param.som_delay;    % time delay for dendritic inhibition from SOM cells
delay_steps = ceil( som_delay / dt);    % transfer time delay to steps
som_inhibition_buffer = zeros(delay_steps, ne);
som_inhibition_buffer_PV = zeros(delay_steps, ni);
...
if isempty(init)
    ...
    vi=zeros(1,ni);
    vs=zeros(1,ns);
    he=zeros(3,ne); % row1: excitation; row2: somatic inhibition;   row3: dendritic inhibition
...
for step = 2:duration/dt
    ...
    if spikecount_e>0
        if ref_e < 0  % ref_e: refractory period
            ve(sind_e) = ve(sind_e) - M;    
        else
            ve(sind_e) = 0; % reset to zero
            refc_e(sind_e) = refc_e(sind_e) + ref_e;    % update reference clock
        end
        ...
        spike_e(1,sind_e) = spike_e(1,sind_e) + 1;  % spike number for each neuron              
...
%         disp(round(spikeind));
    ...
    he(3,:) = he(3,:) + som_inhibition_buffer(1, :);
    hi(3,:) = hi(3,:) + som_inhibition_buffer_PV(1, :);

    % update som_inhibition_buffer
    for buffer_i = 1 : (delay_steps-1)
        som_inhibition_buffer(buffer_i, :) = som_inhibition_buffer(buffer_i+1, :);
        som_inhibition_buffer_PV(buffer_i, :) = som_inhibition_buffer_PV(buffer_i+1, :);
    end
    som_inhibition_buffer(delay_steps, :) = 0;
    som_inhibition_buffer_PV(delay_steps, :) = 0;
    ...
    if spikecount_s>0
         if ref_s < 0
            ...
            vs(sind_s) = 0;
            refc_s(sind_s) = refc_s(sind_s) + ref_s;
         end

        % store the som inhibition to buffer
        som_inhibition_buffer(delay_steps, :) = sum(connection_matrix_s(sind_s, 1:ne), 1);
        som_inhibition_buffer_PV(delay_steps, :) = sum(connection_matrix_s(sind_s, ne+1 : ne+ni), 1);    
